...
def test_bad_manifest_fails_with_categorical_code(
    ...
    yml = (
        ...
        .read_text()
        .replace("{{IRONCLAUDE_SNAPSHOT_PATH}}", str(ironclaude_fixture_repo))
    )
    ...
    assert result.exit_code == expected_code, (
        f"{fixture_name}: expected {expected_code.name}, got {result.exit_code.name}"
    )
...
def test_orphan_command_fails_with_co_import_missing(
    ...
    yml = (
        (FIXTURES / "bad-orphan-command.yaml")
        .read_text()
        .replace("{{IRONCLAUDE_SNAPSHOT_PATH}}", str(ironclaude_fixture_repo))
    )
